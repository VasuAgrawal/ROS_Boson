...
int Send_Serial_package( unsigned char *package, short package_len) {
    // Clear TX buffer
//ROS_INFO("Before flush\n");
    flush_buffer_tx(serial);   // Serial GLobal Var
//ROS_INFO("After flush\n");
...
bool raw(flir_boson::RawCommand::Request  &req,
         flir_boson::RawCommand::Response &res)
{
  //res.sum = req.a + req.b;
  //ROS_INFO("request: x=%s, y=%s", req.command.c_str(), req.data.c_str());
  ...
  //baudios=921600;
  // Gest values from parameter server if not exist we put a default value
  ros::NodeHandle n;
  std::string s;
  n.param<std::string>("/boson_raw_command_server/serial_port", s, "/dev/ttyACM0");
  strcpy(puerto_str, s.c_str());
  n.param<std::string>("/boson_raw_command_server/baudrate", s, "921600");
  baudios = atoi(s.c_str());

  //commandID
  strcpy(commandID_str, req.command.c_str());
  commandID = hex_to_long(commandID_str);

  //FUNCTION DATA
  if (req.data.size() > 0 && req.data.size() < 768) {
    for (int i=0; i<req.data.size(); i=i+2) {
	strcpy(aux_data_str, req.data.substr(i,2).c_str());
  ...
  /* STEP 3 */
  // Send over serial
  ROS_INFO("Enviando trama: %d - %d", num_bytes, boson_stuffed_package_len);
...
char result[2048]="";
for (unsigned int i = 0; i < BosonData_count; i++) {
  sprintf(result, "%s%02x", result, BosonData[i]);
}
//printf("Result: %s", result);
res.output = std::string((char*) result);
...
{
  ros::init(argc, argv, "boson_raw_command_server");
  ros::NodeHandle n;

  ros::ServiceServer service = n.advertiseService("raw_command", raw);
  ROS_INFO("Ready to receive a raw command.");
